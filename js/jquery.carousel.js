;(function($){
   //创建构造函数
   var Carousel=function(element,options, type){
   ...
   Carousel.prototype.setArrow=function(){
      if(this.options.arrow.isshow){
        this.$element.append('<div class="carArrow wi-100"></div>');
   ...
   //自动轮播器
   Carousel.prototype.autoCarousel=function(type){
      ...
      if(this.class === 'slider'){
        ...
        for (var i = 0; i <= divsle; i++) {
            ...
            if(cla === 'item wi-100 select'){
                ...
                $("span[number='"+i+"']").css('color','#a0a0a0');
            }
        }
      }
      else if(this.class === 'crew'){
        let divsle = $(".d").find('.item').length;
        for (var i = 0; i <= divsle; i++) {
            let div = $(".d").find('.item')[i];
            let cla = $(div).attr('class');

            if(cla === 'item wi-100 select'){
                $("#crew-"+i).css('color','#ffff');
            }
            else{
                $("#crew-"+i).css('color','#8c8a8a');
            }
        }
      }
   ...
   //将插件函数绑定到$.fn上，来实现扩展
   $.fn.jCarousel=function(option,type){
      return this.each(function(){
        var options=$.extend({},$.fn.jCarousel.default,option);
        var c=new Carousel(this,options,type);
        c.init(type);
